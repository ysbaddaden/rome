module Rome
  ...
  #
  # A Relation is immutable. All methods will return a copy of the previous
  # Relation with the added constraint(s). Methods are also chainable.
  #
  ...
  # then load many records and cache them into the Relation, so further accesses
  # won't re-execute the SQL. Some methods such as `#first` or `#size` do
  # leverage this cache when it's available.
  #
  # ```
  # users = User.select(:id, :name).where(group_id: 2)
  #
  # first_user = User.order(:name).take
  # # => SELECT "id", "name" FROM "users" WHERE "group_id" = 2 ORDER BY "name" ASC LIMIT 1;
  #
  # users.to_a
  ...
  # properly quoted for the database server.
  #
  # In many cases you can specify raw SQL statements using a String. For
  # example:
  # ```
  # users = User.where("LENGTH(name) > ?", 10)
  # # => SELECT * FROM "users" WHERE LENGTH(name) > 10;
  #
  # users = User.oreder("LENGTH(name) DESC")
  ...
  struct Relation(T)
    ...
    # Loads all records from the database. Fills the cache so the query is only
    # executed once.
    def to_a : Array(T)
    ...
    # See `#to_a` to load all records from the database into an Array.
    def all : self
      self
    end
