...
module Rome
  class Adapter::SQLite3Test < AdapterTest
    protected def adapter(builder = @builder)
      Adapter::SQLite3.new(builder)
    end
    ...
    def test_select_where_regex
      ...
      assert_sql({
        %(SELECT * FROM `users` WHERE `name` REGEXP ? AND `name` REGEXP ?),
        ["jul", "a[bc]"]
      }, adapter.select_sql)
    end
    ...
    def test_select_where_not_regex
      ...
      assert_sql({
        %(SELECT * FROM `users` WHERE NOT (`name` REGEXP ?) AND NOT (`name` REGEXP ?)),
        ["jul", "a[bc]"]
      }, adapter.select_sql)
    end
    ...
    def test_insert
      ...
      assert_sql({
        %(INSERT INTO `suppliers` DEFAULT VALUES), [] of String
